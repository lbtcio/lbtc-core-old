...
static void SendMoneyNew(const CTxDestination &address, CAmount nValue, bool fSubtractFeeFromAmount, CWalletTx& wtxNew)
{
    CAmount curBalance = pwalletMain->GetBalance();
    ...
    CRecipient recipient = {scriptPubKey, nValue, fSubtractFeeFromAmount};
    vecSend.push_back(recipient);
    if (!pwalletMain->CreateTransaction(vecSend, wtxNew, reservekey, nFeeRequired, nChangePosRet, strError, NULL, true, NULL)) {
        if (!fSubtractFeeFromAmount && nValue + nFeeRequired > curBalance)
            strError = strprintf("Error: This transaction requires a transaction fee of at least %s", FormatMoney(nFeeRequired));
        throw JSONRPCError(RPC_WALLET_ERROR, strError);
    }
...
UniValue sendtoaddressnew(const JSONRPCRequest& request)
{
    if (!EnsureWalletIsAvailable(request.fHelp))
        return NullUniValue;
    ...
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 5)
        throw runtime_error(
            ...
            "\"txid\"                  (string) The transaction id.\n"
            "\nExamples:\n"
            + HelpExampleCli("sendtoaddressnew", "\"1M72Sfpbz1BPpXFHz9m3CdqATR44Jvaydd\" 0.1")
            + HelpExampleCli("sendtoaddressnew", "\"1M72Sfpbz1BPpXFHz9m3CdqATR44Jvaydd\" 0.1 \"donation\" \"seans outpost\"")
            + HelpExampleCli("sendtoaddressnew", "\"1M72Sfpbz1BPpXFHz9m3CdqATR44Jvaydd\" 0.1 \"\" \"\" true")
            + HelpExampleRpc("sendtoaddressnew", "\"1M72Sfpbz1BPpXFHz9m3CdqATR44Jvaydd\", 0.1, \"donation\", \"seans outpost\"")
        );
...
static void SendRegiste(const CBitcoinAddress &address, const std::string& name, CWalletTx& wtxNew)
{
    CAmount curBalance = pwalletMain->GetBalance();
    std::string strError;

    CCoinControl coinControl;
    coinControl.nMinimumTotalFee = 100000000;
    ...
    CScript registeScript(OP_REGISTE);
    auto vName = ToByteVector(name);
    vName.resize(32);
    registeScript << vName;

    vector<unsigned char> vote_data = pwalletMain->CreateVoteInfo(address.Get(), ToByteVector(registeScript));
    if(vote_data.empty()) {
    ...
    vector<CRecipient> vecSend;
    if ( !pwalletMain->CreateTransaction(vecSend, wtxNew, reservekey, nFeeRequired, nChangePosRet, strError, &coinControl, true, &vote_data)) {
        strError = "SendRegiste CreateTransaction error";
        throw JSONRPCError(RPC_WALLET_ERROR, strError);
    }
...
UniValue registe(const JSONRPCRequest& request)
{
    ...
    if(Vote::GetInstance().HaveDelegate(request.params[1].get_str(), delegate)) {
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, string("delegate data error"));
    }
...
static void SendVote(const CBitcoinAddress& fromAddr, const set<CBitcoinAddress>& setAddress, CWalletTx& wtxNew, bool isVote)
{
    ...
    opcodetype op_code = OP_VOTE;
    if(isVote == false) {
        op_code = OP_REVOKE;    
    }
    CScript scriptPubKey(op_code);

    unsigned char s_key[Vote::MaxNumberOfVotes * 20];
    unsigned char *p_s_key = &s_key[0];
    for (auto& a: setAddress) {
        CKeyID id;
        a.GetKeyID(id);
        memcpy(p_s_key, id.begin(), 20);
        p_s_key += 20;
    }
       scriptPubKey << std::vector<unsigned char>(&s_key[0], p_s_key);

    vector<unsigned char> vote_data = pwalletMain->CreateVoteInfo(fromAddr.Get(), ToByteVector(scriptPubKey));
    if(vote_data.empty()) {
        strError = strprintf("Error: CreateVoteInfo");
        throw JSONRPCError(RPC_WALLET_ERROR, strError);
    }
    ...
    vector<CRecipient> vecSend;
    if ( !pwalletMain->CreateTransaction(vecSend, wtxNew, reservekey, nFeeRequired, nChangePosRet, strError, &coinControl, true, &vote_data)) {
        strError = "SendVote CreateTransaction error";
        throw JSONRPCError(RPC_WALLET_ERROR, strError);
    }

    CValidationState state;
    if (!pwalletMain->CommitTransaction(wtxNew, reservekey, g_connman.get(), state)) {
        for(auto& vi:wtxNew.tx->vin)
        {
            std::cout << "reject tx," << vi.prevout.hash.ToString() << "," << vi.prevout.n << std::endl;
        }

        for(auto& vo:wtxNew.tx->vout){
            std::cout << "reject tx," << vo.nValue << std::endl;
        }
        strError = strprintf("Error: The transaction was rejected! Reason given: %s", state.GetRejectReason());
        throw JSONRPCError(RPC_WALLET_ERROR, strError);
    }
...
{ //  category              name                        actor (function)           okSafeMode
    ...
    { "wallet",             "sendtoaddressnew",         &sendtoaddressnew,         false,  {"address","amount","comment","comment_to","subtractfeefromamount"} },
