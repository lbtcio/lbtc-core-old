...
//std::map<uint256, std::vector<CTxOut>> mapHashTxout;
std::unordered_map<uint256, std::vector<CTxOut>, uint256_hash> mapHashTxout(500000);

bool DoVoting(const CBlock& block, uint32_t nHeight, std::map<uint256, uint64_t>& mapTxFee);
bool UndoVoting(const CBlock& block, uint32_t nHeight, std::map<uint256, uint64_t>& mapTxFee);
                              ...
                              bool fOverrideMempoolLimit, const CAmount& nAbsurdFee, std::vector<uint256>& vHashTxnToUncache)
{
    ...
    if(tx.nVersion < CTransaction::DPOS_VERSION)
        return false;
...
bool ReadBlockFromDisk(CBlock& block, const CDiskBlockPos& pos, const Consensus::Params& consensusParams)
{
    ...
    // Check the header
    if (!CheckProofOfWork(block.hashPrevBlock, block.GetHash(), block.nBits, consensusParams))
...
CAmount GetBlockSubsidyNew(int nHeight, const Consensus::Params& consensusParams)
{
    CAmount nSubsidy = 0.0625 * COIN;
    if(nHeight > 26500000) {
        int halvings = (nHeight - 26500000) / 42000000;
        if(halvings >= 61) {
            return 0;
        }
...
{
    if(nHeight >= 679999) {
        return GetBlockSubsidyNew(nHeight, consensusParams);    
    }
    ...
    //CAmount nSubsidy = 50 * COIN / 200;
    // Subsidy is cut in half every 210,000 blocks which will occur approximately every 4 years.
    nSubsidy >>= halvings;

    if(nHeight >= DPoS::GetInstance().GetForkHeight()) {
        nSubsidy /= 200;    
    }
...
void CheckForkWarningConditions()
{
    ...
    // of our head, drop it
    if (pindexBestForkTip && chainActive.Height() - pindexBestForkTip->nHeight >= 72 * 200)
        pindexBestForkTip = NULL;

    if (pindexBestForkTip || (pindexBestInvalid && pindexBestInvalid->nChainWork > chainActive.Tip()->nChainWork + (GetBlockProof(*chainActive.Tip()) * 6 * 200)))
    {
...
bool CScriptCheck::operator()() {
    ...
    if (DPoS::GetInstance().IsInitMode() == false && !VerifyScript(scriptSig, scriptPubKey, witness, nFlags, CachingTransactionSignatureChecker(ptxTo, nIn, amount, cacheStore, *txdata), &error)) {
        return false;
    }
    return true;
}
...
bool CheckTxInputs(const CTransaction& tx, CValidationState& state, const CCoinsViewCache& inputs, int nSpendHeight)
{
        ...
        for (unsigned int i = 0; i < tx.vin.size(); i++)
        {
...

            {
                ...
                mapHashTxout.insert(make_pair(prevout.hash, coins->vout));
            }

            // If prev is coinbase, check that it's matured
            if (coins->IsCoinBase()) {
                if(nSpendHeight < 500000)  {
                    if (nSpendHeight - coins->nHeight < 100)
                        return state.Invalid(false,
                ...
                if (fAddressIndex) {
                    ...
                    if (ExtractDestination(prevout.scriptPubKey, address)) {
                        if(address.type() == typeid(CKeyID)) {
                            addressIndex.push_back(std::make_pair(CAddressIndexKey(1, boost::get<CKeyID>(address), pindex->nHeight, i, txhash, j, false), prevout.nValue));
                        } else if(address.type() == typeid(CScriptID)) {
                            addressIndex.push_back(std::make_pair(CAddressIndexKey(2, boost::get<CScriptID>(address), pindex->nHeight, i, txhash, j, false), prevout.nValue));
                        } else {
                            continue;
                        }
                    }
        ...
        for (unsigned int k = 0; k < tx.vout.size(); k++) {
            if (fAddressIndex) {
                ...
                if (ExtractDestination(out.scriptPubKey, address)) {
                    if(address.type() == typeid(CKeyID)) {
                        addressIndex.push_back(std::make_pair(CAddressIndexKey(1, boost::get<CKeyID>(address), pindex->nHeight, i, txhash, k, false), out.nValue));
                    } else if(address.type() == typeid(CScriptID)) {
                        addressIndex.push_back(std::make_pair(CAddressIndexKey(2, boost::get<CScriptID>(address), pindex->nHeight, i, txhash, k, false), out.nValue));
                    } else {
                        continue;
                    }
                }
...
int32_t ComputeBlockVersion(const CBlockIndex* pindexPrev, const Consensus::Params& params)
{
    ...
    if(pindexPrev && pindexPrev->nHeight >= 894177)
        nVersion |= (((uint32_t)1) << 16);
...
void static UpdateTip(CBlockIndex *pindexNew, const CChainParams& chainParams) {
    ...
    LogPrintf("%s: new best=%s height=%d version=0x%08x tx=%lu date='%s' cache=%.1fMiB(%utx)", __func__,
            ...
            if (fFailedChain || fMissingData) {
                ...
                if (fFailedChain &&
                                (pindexBestInvalid == NULL
                                    ...
                                    || (pindexBestInvalid->nChainWork == pindexNew->nChainWork && pindexNew->nTime > pindexNew->nTime)))
                    pindexBestInvalid = pindexNew;
...
CBlockIndex* AddToBlockIndex(const CBlockHeader& block)
{
    ...
    if (pindexBestHeader == NULL
        ...
        || (pindexBestHeader->nChainWork == pindexNew->nChainWork && pindexNew->nTime > pindexBestHeader->nTime))
...
bool CheckBlockHeader(const CBlockHeader& block, CValidationState& state, const Consensus::Params& consensusParams, bool fCheckPOW)
{
    ...
    if (fCheckPOW && !CheckProofOfWork(block.hashPrevBlock, block.GetHash(), block.nBits, consensusParams))
...
bool IsVotingTxout(const CTxOut& txout, CPubKey& pubkey, uint64_t& nTime, CScript& script) 
{
    ...
    ++iter;
    if (!txout.scriptPubKey.GetOp2(iter, opcode, &vchRet))
    {
        LogPrint("DPoS", "get vch failed\n");
        return false;
    }
    ...
    data.reserve(vchRet.size());
    CScript s(vchRet.begin(), vchRet.end());
    iter = s.begin();
    vchRet.clear();
    if (!s.GetOp2(iter, opcode, &vchRet) ) // for hash
    {
        LogPrint("DPoS", "get hash error\n");
        return false;
    }
    if(vchRet[0] != 'L'
        || vchRet[1] != 'B'
        || vchRet[2] != 'T'
        || vchRet[3] != 'C') {
        LogPrint("DPoS", "get vote symbole failed\n");
        return false;    
    }

    CHashWriter ss(SER_GETHASH, 0);
    if (!s.GetOp2(iter, opcode, &vchRet) ) // for pubkey
    {
        LogPrint("DPoS", "get pubkey failed\n");
        return false;
    }
    ss << vchRet;
    pubkey.Set(vchRet.begin(), vchRet.end());
    if (!pubkey.IsValid())
    {
        LogPrint("DPoS", "get pubkey is invalid\n");
        return false;
    }
    std::copy(vchRet.begin(), vchRet.end(), std::back_inserter(data));
    if (!s.GetOp2(iter, opcode, &vchRet) && vchRet.size() != 8) // for timestamp
    {
        LogPrint("DPoS", "get timestamp failed\n");
        return false;
    }
    nTime = set_vch(vchRet);
    ss << vchRet;
    auto hTimestamp = Hash(vchRet.begin(), vchRet.end());
    std::copy(vchRet.begin(), vchRet.end(), std::back_inserter(data));
    if (!s.GetOp2(iter, opcode, &vchRet) ) // for sig
    {
        LogPrint("DPoS", "get sign failed\n");
        return false;
    }
    if (!pubkey.Verify(hTimestamp, vchRet))
    {
    ...
    ss << vchRet;
    std::copy(vchRet.begin(), vchRet.end(), std::back_inserter(data));
    std::vector<unsigned char> vchSig(vchRet.begin(), vchRet.end());
    if (vchSig.empty())
    {
        LogPrint("DPoS", "get vchsign empty\n");
        return false;
    }
    if (!s.GetOp2(iter, opcode, &vchRet) ) // for data
    {
        LogPrint("DPoS", "get data failed\n");
        return false;
    }
    ss << vchRet;
    std::copy(vchRet.begin(), vchRet.end(), std::back_inserter(data));

    script = CScript(vchRet.begin(), vchRet.end());

    return true;
};

bool ProcessRegiste(uint32_t nHeight, const CBlock& block, const CPubKey& pubkey, uint64_t nTime, const CScript& script)
{
    if (script[0] != OP_REGISTE)
        return false;
    ...
    //std::string name((const char*)vchRet.data(), strlen(vchRet.size());
    std::string name;
    for(auto i : vchRet) {
        if(i == '\0')
            break;
    ...
    bool ret = Vote::GetInstance().ProcessRegister(delegate, name);
    if(ret) {
        LogPrintf("DPoS Register: Height:%u %s ---> %s success\n", nHeight, CBitcoinAddress(delegate).ToString().c_str(), name.c_str());    
    } else {
        LogPrintf("DPoS Register: Height:%u  %s ---> %s fail\n", nHeight, CBitcoinAddress(delegate).ToString().c_str(), name.c_str());    
    }

    return ret;
}

bool ProcessUnregiste(uint32_t nHeight, const CBlock& block, const CPubKey& pubkey, uint64_t nTime, const CScript& script)
{
    //if (script[0] != OP_REGISTE)
        //return false;
    ...
    CKeyID delegate = pubkey.GetID();
    std::string name((const char*)vchRet.data(), vchRet.size());

    bool ret = Vote::GetInstance().ProcessUnregister(delegate, name);
    if(ret) {
        LogPrintf("DPoS Unregister: Height:%u %s ---> %s success\n", nHeight, CBitcoinAddress(delegate).ToString().c_str(), name.c_str());    
    } else {
        LogPrintf("DPoS Unregister: Height:%u  %s ---> %s fail\n", nHeight, CBitcoinAddress(delegate).ToString().c_str(), name.c_str());    
    }
    return ret;
}

bool ProcessRevoke(uint32_t nHeight, const CBlock& block, const CPubKey& pubkey, uint64_t nTime, const CScript& script)
{
    auto iter = script.begin();
    ++iter;
    opcodetype opcode;
    ...
    bool ret = Vote::GetInstance().ProcessCancelVote(key, setDelegate);
    if(ret) {
        LogPrintf("DPoS CancelVote Height:%u\n", nHeight);
        for(auto i : setDelegate) {
            LogPrintf("DPoS CancelVote: %s ---> %s(%s)\n", CBitcoinAddress(key).ToString().c_str(), Vote::GetInstance().GetDelegate(i).c_str(), CBitcoinAddress(i).ToString().c_str());    
        }

        LogPrintf("DPoS CancelVote success\n");
    } else {
        LogPrintf("DPoS CancelVote Height:%u\n", nHeight);
        for(auto i : setDelegate) {
            LogPrintf("DPoS CancelVote: %s ---> %s(%s)\n", CBitcoinAddress(key).ToString().c_str(), Vote::GetInstance().GetDelegate(i).c_str(), CBitcoinAddress(i).ToString().c_str());    
        ...
        LogPrintf("DPoS CancelVote fail\n");
    }

    return ret;
}

bool ProcessVote(uint32_t nHeight, const CBlock& block, const CPubKey& pubkey, uint64_t nTime, const CScript& script)
{
    auto iter = script.begin();
    ++iter;
    opcodetype opcode;
    ...
    bool ret = Vote::GetInstance().ProcessVote(key, setDelegate);
    if(ret) {
        LogPrintf("DPoS Vote Height:%u\n", nHeight); 
        for(auto i : setDelegate) {
            LogPrintf("DPoS Vote: %s ---> %s(%s)\n", CBitcoinAddress(key).ToString().c_str(), Vote::GetInstance().GetDelegate(i).c_str(), CBitcoinAddress(i).ToString().c_str());    
        }

        LogPrintf("DPoS Vote success\n");
    } else {
        LogPrintf("DPoS Vote Height:%u\n", nHeight); 
        for(auto i : setDelegate) {
            LogPrintf("DPoS Vote: %s ---> %s(%s)\n", CBitcoinAddress(key).ToString().c_str(), Vote::GetInstance().GetDelegate(i).c_str(), CBitcoinAddress(i).ToString().c_str());    
            ...
            switch (script[0]) {
                case OP_REGISTE:
                    if(mapTxFee[t->GetHash()] >= 100000000)
                        ProcessRegiste(nHeight, block, pubkey, nTime, script);
                break;
                 
                case OP_VOTE:
                    if(mapTxFee[t->GetHash()] >= 1000000)
                        ProcessVote(nHeight, block, pubkey, nTime, script);
                break;
                    
                case OP_REVOKE:
                    if(mapTxFee[t->GetHash()] >= 1000000)
                        ProcessRevoke(nHeight, block, pubkey, nTime, script);
                break;
            }
        }
    }
...
bool UndoVoting(const CBlock& block, uint32_t nHeight, std::map<uint256, uint64_t>& mapTxFee) 
{
    ...
    for(auto it = block.vtx.rbegin(); it != block.vtx.rend(); ++it) {
        ...
        if (IsVotingTxout(to, pubkey, nTime, script))
        {
            ...
            switch (script[0]) {
                case OP_REGISTE:
                        ProcessUnregiste(nHeight, block, pubkey, nTime, script);
                break;
...
void ProcessDPoSConnectBlock(const CBlock& block, uint64_t nBlockHeight)
{
    ...
    DoVoting(block, nBlockHeight, mapTxFee);
}
...
void ProcessDPoSDisconnectBlock(const CBlock& block, uint64_t nBlockHeight)
{
    ...
    CalculateBalance(block, false, NULL);
    std::map<uint256, uint64_t> mapTxFee;
    UndoVoting(block, nBlockHeight, mapTxFee);
}
...
bool GetTxoutSlow(CTxOut& txout, const uint256 &txid, uint32_t nTxoutIndex)
{
    bool ret = false;
    CTransactionRef tx;
    uint256 hashBlock;
...
bool GetTxoutFast(CTxOut& txout, const uint256 &txid, uint32_t nTxoutIndex)
{
    bool ret = false;
    LOCK(cs_mapHashTxout);
    ...
    if(it != mapHashTxout.end()) {
        if(nTxoutIndex < it->second.size()) {
            txout = it->second[nTxoutIndex];
            ret = true;
        }
    }
...
{
    if(GetTxoutFast(txout, txid, nTxoutIndex) == false) {
        return GetTxoutSlow(txout, txid, nTxoutIndex);
    }
    return true;
}
