...
void DPoS::Init()
{
    ...
    if(Params().NetworkIDString() == "main") {
        strDelegateAddress = "166D9UoFdPcDEGFngswE226zigS8uBnm3C";
        gDPoS.nDposStartTime = 1517763452;
...
bool DPoS::IsInitMode()
{
    ...
    bool ret = false;
    if(chainActive.Height() + 1 >= nForkHeight && chainActive.Height() < 500113) {
        ret = true;
    }
...
bool DPoS::CheckTransactionVersion(const CBlock& block)
{
    for(auto& i : block.vtx)
        if(i->nVersion < CTransaction::DPOS_VERSION)
            return false;
