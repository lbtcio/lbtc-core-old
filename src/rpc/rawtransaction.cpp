...
void TxToJSONNew(const CTransaction& tx, const uint256 hashBlock, UniValue& entry)
{
    entry.push_back(Pair("txid", tx.GetHash().GetHex()));
    entry.push_back(Pair("hash", tx.GetWitnessHash().GetHex()));
    entry.push_back(Pair("size", (int)::GetSerializeSize(tx, SER_NETWORK, PROTOCOL_VERSION)));
    ...
    for (unsigned int i = 0; i < tx.vout.size(); i++) {
        ...
        time_t t;
        if(txout.nValue == 0 && DPoS::ScriptToDelegateInfo(cDelegateInfo, t, NULL, txout.scriptPubKey)) {
            if(cDelegateInfo.delegates.empty() == false) {
                UniValue delegates(UniValue::VARR);
                for(auto& i : cDelegateInfo.delegates) {
                    delegates.push_back(CBitcoinAddress(i.keyid).ToString());
                }

                out.push_back(Pair("type", "CoinbaseDelegateInfo"));
                out.push_back(Pair("delegates", delegates));
            }
        }
        vout.push_back(out);
    }
...
UniValue gettransactionnew(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 2)
        throw runtime_error(
            "gettransactionnew \"txid\"\n"
            ...
            "\nExamples:\n"
            + HelpExampleCli("gettransactionnew", "\"mytxid\"")
            + HelpExampleRpc("gettransactionnew", "\"mytxid\"")
        );
...
{ //  category              name                      actor (function)         okSafeMode
  //  --------------------- ------------------------  -----------------------  ----------
    ...
    { "rawtransactionsnew", "gettransactionnew",      &gettransactionnew,      true,  {"txid","verbose"} },
